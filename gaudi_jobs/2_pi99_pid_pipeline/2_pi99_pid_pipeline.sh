#!/bin/bash
# PID 2026 pipeline — pi- beam at 99 GeV, beam centre (1.139, 1.164) mm, 50k events/energy.
#
...
#
# Usage:  bash gaudi_jobs/2_pi99_pid_pipeline/2_pi99_pid_pipeline.sh [--allow-partial]
set -uo pipefail
exec bash "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/pid2026_common/pipeline_common.sh" \
    --particle pi- --energy 99 "$@"
