#!/bin/bash
# Pipeline for 52 GeV electron beam simulation
...
SIM_FILE="${EOS_BASE}/Generated/output_beam_e-_52GeV_xy_-45_45_sigx13.75_sigy8.25_sigE0.02.edm4hep.root"
LABEL=$(basename "${SIM_FILE}" .edm4hep.root | sed 's/^output_//')
PROCESSED="${EOS_BASE}/Processed"

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "${REPO_ROOT}/gaudi_jobs/1_e52_beam_pipeline"
...
INPUT_FILE="digitized.edm4hep.root" INPUT_COLLECTION="SiPadHitsDigi" \
    OUTPUT_FILE="tracks.edm4hep.root" SEED_MOMENTUM=52.0 \
...
bash analysis/run_pid_sim.sh \
    --input     "${REPO_ROOT}/gaudi_jobs/1_e52_beam_pipeline/digitized.edm4hep.root" \
    --ecal-tree "${REPO_ROOT}/gaudi_jobs/1_e52_beam_pipeline/ecal_sim.root" \
    --outdir    "${REPO_ROOT}/gaudi_jobs/1_e52_beam_pipeline" \
    --format    both 2>&1 | grep -v "^TCling::LoadPCM"

echo "=== Moving outputs to Processed (EOS only) ==="
mv "${REPO_ROOT}/gaudi_jobs/1_e52_beam_pipeline/digitized.edm4hep.root" \
   "${PROCESSED}/${LABEL}_digitized.edm4hep.root"
mv "${REPO_ROOT}/gaudi_jobs/1_e52_beam_pipeline/tracks.edm4hep.root" \
   "${PROCESSED}/${LABEL}_tracks.edm4hep.root"
for ext in root edm4hep.root valtree.root; do
    src="${REPO_ROOT}/gaudi_jobs/1_e52_beam_pipeline/ecal_sim.${ext}"
