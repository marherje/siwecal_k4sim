#!/bin/bash
# PID 2026 pipeline — mu- merged 74+99 GeV sample, beam centre (1.139, 1.164) mm, 50k events/energy.
#
...
#
# Usage:  bash gaudi_jobs/2_mumerged_pid_pipeline/2_mumerged_pid_pipeline.sh [--allow-partial]
set -uo pipefail
exec bash "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/pid2026_common/pipeline_common.sh" \
    --particle mu- --energy merged "$@"
